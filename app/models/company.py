...
class Company(Base):
    ...
    threshold_cvss: Mapped[float] = mapped_column(
        Numeric(3, 1), nullable=False, default=7.0
    )
    ...
    __table_args__ = (
        CheckConstraint(
            "threshold_cvss >= 0 AND threshold_cvss <= 10",
            name="companies_threshold_cvss_range_check",
        ),
    )
